...
class KalmanFilter(object):
    ...
    def _jacobian_h(self, x):
        ...
        H = np.zeros((self.ndim, 2 * self.ndim))
        H[: self.ndim, : self.ndim] = np.eye(self.ndim)
        return H
    ...
    def predict(self, mean, covariance):
        ...
        # speed = np.linalg.norm(mean[4:6])  # Speed is calculated from velocity components
        ...
        covariance = np.dot(np.dot(F, covariance), F.T) + motion_cov
    ...
    def project(self, mean, covariance):
        ...
        covariance = np.dot(np.dot(H, covariance), H.T) + innovation_cov
    ...
    def update(self, mean, covariance, measurement):
        ...
        kalman_gain = scipy.linalg.cho_solve(
            (chol_factor, lower),
            np.dot(covariance, self._jacobian_h(mean).T).T,
            check_finite=False,
        ).T
        ...
        new_covariance = covariance - np.dot(
            kalman_gain, np.dot(projected_cov, kalman_gain.T)
        )
